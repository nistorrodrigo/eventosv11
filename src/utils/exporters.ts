...
import { getMeetingAddress, applyBATraffic, PLATFORM_LABELS, PLATFORM_ICONS, officeAddressOf, officeNameOf } from "../travel.js";
...
export function _exportRoadshowSummary({roadshow, openPrint, tz=BASE_TZ}){
  ...
  const tzBannerHTML=isOtherTz?`<div style="background:#eff6ff;border-left:3px solid #3399ff;border-radius:4px;padding:9px 14px;font-size:9.5pt;color:#1e5ab0;margin-bottom:16px;-webkit-print-color-adjust:exact;print-color-adjust:exact">⏰ <strong>All times shown in ${tzLabel}${tzOffset?` (${tzOffset})`:""}.</strong></div>`:"";
  const {trip,meetings,companies}=roadshow;
  const rsCoMap=new Map((companies||[]).map(c=>[c.id,c]));
  ...
  // tz-aware hour formatter — when tz is non-BA, uses Intl.DateTimeFormat under the hood
  const fmtH=isOtherTz?((h,date)=>fmtHourInTZ(date,h,tz)):fmtHour;
...
${tzBannerHTML}<div class="sec-title">Meeting Schedule</div>${dayRows}
${days.some(d=>byDay[d].some(m=>m.postNotes))?`<div class="sec-title">Post-Meeting Notes</div>${days.flatMap(d=>byDay[d].filter(m=>m.postNotes).map(m=>{const co=m.type==="company"?rsCoMap.get(m.companyId):null;return `<div style="margin-bottom:12px;padding:10px 14px;border-left:3px solid #166534;background:#f0fdf4;border-radius:0 6px 6px 0"><div style="font-weight:600;color:#000039;margin-bottom:4px">${co?co.name:(m.lsType||m.title||"Interno")} · ${fmtDate(m.date)} ${fmtH(m.hour,m.date)}</div><div style="font-size:10pt;color:#166534;line-height:1.6">${m.postNotes}</div></div>`;})).join("")}`:""}
...
export function _exportCompanyBrief({co, roadshow, config, openPrint}){
  ...
  const locStr=!mtg?"TBD":mtg.location==="virtual"?((PLATFORM_ICONS[mtg.meetingPlatform]||"💻")+" "+(PLATFORM_LABELS[mtg.meetingPlatform]||"Virtual meeting")+(mtg.meetingLink?" — "+mtg.meetingLink:"")):mtg.location==="ls_office"?officeAddressOf(mtg,trip):mtg.location==="hq"?(co.hqAddress||co.name+" HQ"):(mtg.locationCustom||"TBD");
...
export function _exportPostRoadshowReport({roadshow, openPrint, tz=BASE_TZ}){
  const isOtherTz=tz!==BASE_TZ;
  const tzLabel=(TIMEZONES.find(t=>t.value===tz)?.label||tz).replace(/^[^ ]+ /,"");
  const tzShort=TIMEZONES.find(t=>t.value===tz)?.short||"local";
  const tzOffset=isOtherTz?tzOffsetLabel(tz):"";
  const tzBannerHTML=isOtherTz?`<div style="background:#eff6ff;border-left:3px solid #3399ff;border-radius:4px;padding:9px 14px;font-size:9.5pt;color:#1e5ab0;margin:0 0 16px;-webkit-print-color-adjust:exact;print-color-adjust:exact">⏰ <strong>All times shown in ${tzLabel}${tzOffset?` (${tzOffset})`:""}.</strong></div>`:"";
  const {trip,meetings,companies}=roadshow;
  const coMap=new Map((companies||[]).map(c=>[c.id,c]));
  const allMtgs=(meetings||[]).filter(m=>m.status!=="cancelled"&&m.type==="company");
  // tz-aware hour formatter
  const fmtH=isOtherTz?((h,date)=>fmtHourInTZ(date,h,tz)):fmtHour;
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
  <div><span style="font-family:'IBM Plex Mono',monospace;font-size:10pt;color:#6b7280;margin-right:8px">${fmtH(m.hour,m.date)}</span><strong style="color:#000039;font-size:11pt">${co?.name||"—"}</strong>${co?.ticker?` <span style="color:#9ca3af;font-size:8.5pt">(${co.ticker})</span>`:""}</div>
...
${mtgs.map(m=>{const co=coMap.get(m.companyId);return `<div class="fu-card"><strong>${co?.name||"—"}</strong> · ${fmtDate(m.date)} ${fmtH(m.hour,m.date)}${m.postNotes?` — <span style="color:#374151">${m.postNotes.slice(0,100)}</span>`:""}</div>`;}).join("")}
</div>`).join("")}`:"";

  // Cover page (same brand treatment as agenda + summary)
  const visitorsForCover=(trip.visitors||[]).filter(v=>v.name).map(v=>({name:v.name,title:v.title||""}));
  const coverDateLabel=(()=>{const d=trip.arrivalDate||trip.departureDate;if(!d) return "";return new Date(d+"T12:00:00").toLocaleDateString("en-US",{month:"long",year:"numeric"});})();
  const coverHTML=buildCoverHTML({title:fund,subtitle:"Post-Roadshow Report",names:visitorsForCover,dateLabel:coverDateLabel});
  const html=`<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>${fund} Post-Roadshow Report${isOtherTz?" "+tzShort:""}</title><style>${COVER_CSS}${css}</style></head><body>
${coverHTML}
${tzBannerHTML}
  ...
  const dayBlocks=days.map(date=>{
    const rows=byDay[date].map((m,idx)=>{
      ...
      const loc=isVirt
        ...
        :`📍 ${escH(m.location==="ls_office"?officeAddressOf(m,trip):m.location==="hq"?(co?.hqAddress||co?.name+" HQ"||"HQ"):(m.locationCustom||"TBD"))}`;
      // Reps
      const reps=(()=>{
        if(m.type==="company"){
