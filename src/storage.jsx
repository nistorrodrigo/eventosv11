...
/* Latin Securities brand artwork — official PNGs in /public.
   They live as files (not inlined base64) so the JS bundle stays small.
   Reference them by absolute URL so the PDF preview window (which has
   about:blank as its initial URL) can resolve them against the app origin. */
const _ORIGIN = typeof window !== 'undefined' ? window.location.origin : '';
export const LS_LOGO_POSITIVE_URL = _ORIGIN + "/ls-logo-positive.png";
export const LS_LOGO_NEGATIVE_URL = _ORIGIN + "/ls-logo-negative.png";
export const LS_LOGO_DATA_URL = LS_LOGO_POSITIVE_URL; // backward-compat alias
...
//
// This is the ORIGINAL working version — the user generated the Blackrock
// roadshow PDFs successfully with this exact flow. The "improvements" we
// tried after (Blob URL, auto-print, inline toolbar) broke Save-as-PDF
// in some browsers. Sticking with what's proven.
//
// Flow:
//   - new tab via window.open("", "_blank")
//   - document.write(html) + close
//   - user manually hits Ctrl+P → Save as PDF
//
// Fallback: if the popup is blocked, download the HTML so the user can
// open it locally and print from there.
...
export function buildPrintHTML(entities,meta={}){
  ...
  // Build doc title for browser tab + default save-as filename
  // Format: "{Fund} Schedule {Month Day Year}" e.g. "Blackrock Schedule June 3 2026"
  const docTitle=(()=>{
    const e0=entities[0]||{};
    const fund=meta.eventTitle||e0.coverTitle?.replace(/\s+Roadshow\s*$/i,"")||"Latin Securities";
    const dateStr=meta.eventDates?String(meta.eventDates).replace(/[,]/g,""):"";
    return `${fund} Schedule${dateStr?" "+dateStr:""}`.trim();
  })();
