...
import { exportHistoricalHTML, _exportExcel } from "./src/utils/exporters.js";
...
<div class="footer"><span>Latin Securities · Confidential</span><span>${co.name} · ${trip.fund||trip.clientName||""}</span></div>
  ...
  function exportDriverItinerary(filterDate){
    ...
    const dayBlocks=days.map(date=>{
      ...
      dayMtgs.forEach((m,mi)=>{
        ...
        if(mi<dayMtgs.length-1){
          ...
          const travelSource=tData?.source==="osrm+traffic"?" (tráfico CABA est.)":(tData?" (sin tráfico)":"");
    ...
    const travelNote=Object.keys(travelCache).length?
      ...
      "⚠ Tiempos de traslado no calculados — hacé click en 🔄 Calcular en el tab 🗺️ Recorrido para ver rangos de tráfico.";
